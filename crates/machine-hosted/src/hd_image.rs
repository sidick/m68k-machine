//! File-backed [`BlockDevice`] for `hostblk` (`--hostblk <path>`).
//!
//! `machine-core` is `#![no_std]` and has no file I/O of its own --
//! `block.rs`'s `BlockDevice` trait doc comment says as much -- so this is
...
//! with no transformation -- a `.hdf` file's on-disk layout and this
//! device's `buf` argument are the same bytes in the same order. This
//! type originally sat behind Gayle's IDE port, whose author's handover
//! explained why that was the correct behaviour even though the CPU
//! sees a byte-swapped data register (the swap is internal to Gayle's
//! own PIO path and cancels out for opaque sector bytes); Gayle is
//! retired (`docs/device-ledger.md`), but the same reasoning carries
//! over unchanged to `hostblk`, which never swaps sector bytes at all.
...
impl FileBlockDevice {
    ...
    ///
    /// Read-only unless `writable` is set -- see `cli.rs`'s
    /// `--hostblk-writable` doc comment for the full reasoning. Short
    /// version: the images worth attaching (an amibake conversion of
    /// licensed media, `docs/storage.md`) took real effort to build and
    /// are not redistributable if lost, and a boot that never gets to
    /// write a sector is still a fully useful milestone. Opt-in write
    /// access is one flag away when it is actually needed.
    ///
...
impl BlockDevice for FileBlockDevice {
    ...
    /// out-of-range LBA -- `hostblk` turns that into a clean error
