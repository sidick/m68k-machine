...
- **Any x86 desktop:** same image under KVM; mature VFIO hosts the **PCIe-to-PCI bridge + period cards rig** (Voodoo3/RTL8139/SB128, a PCI USB (OHCI/UHCI) controller card vs shipped P96 `.card`s and era drivers) = Prometheus API conformance for `pci.library`. The USB card doubles as a real-hardware precursor to Phase 6's xHCI HID work (proposal's USB HID row) — worth noting whether a period PCI USB `.device` stack runs unmodified through this library before any xHCI driver of our own is written.
  ...
  **The mechanism, spelled out because it is easy to underestimate how cheap this is:** VFIO passthrough is transparent to the guest — a physical card bound to the host's `vfio-pci` driver and attached via `qemu ... -enable-kvm -device vfio-pci,host=<BDF>` just appears in the guest's normal ECAM config space and BAR windows; the host IOMMU handles isolation and DMA remapping underneath, and no VFIO-aware code is needed on the guest side, only an ordinary working PCI driver. `board-qemu-q35`/`board-qemu-virt` already boot as a QEMU guest's "kernel"; running the same payload with `-enable-kvm` (a genuine KVM guest, not bare metal) plus a `PciBackend` implementation against that board's own real ECAM window (stage 1's recorded deferral) is what lets a VFIO-passed-through card show up on the guest's PCI bus. That makes the whole period-card conformance rig — and real driver development against physical PCIe silicon generally — reachable from an ordinary x86 dev box with an IOMMU, months before any bare-metal UEFI/bootloader bring-up is needed. Worth building this path first, specifically because it is so much cheaper than physical hardware bring-up.
