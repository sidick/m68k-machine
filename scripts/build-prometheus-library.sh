#!/usr/bin/env bash
# Rebuilds m68k/prometheus-library/prometheus.library from
# m68k/prometheus-library/{libstart.S,prometheus_library.c}, the
# Prometheus-compatible PCI bus library for the `pcibridge` device
# (docs/pcibridge-protocol.md, docs/pci-library.md).
#
# NOT part of the Rust build and NOT run by CI -- this is a real AmigaOS
# AUTOINIT disk library (LIBS:prometheus.library, opened by name like any
# other shared library), not something crates/machine-core or
...
# rtgboard-card.sh's own header comment, scripts/build-pktport-handler.sh,
# assets/hostblk-rom/hostblk-diagrom.bin) so that anyone testing this
# library doesn't need a working m68k toolchain just to run it.
#
...
# No external SDK is needed beyond this toolchain's own bundled NDK
# headers (ndk-include, the default -I search path) -- unlike rtgboard's
# P96 SDK dependency, prometheus.library's public headers are this
# project's own (m68k/prometheus-library/include/**).
...
SRC_DIR="$REPO_ROOT/m68k/prometheus-library"
OUT="$SRC_DIR/prometheus.library"
...
# Compile flags copied from rtgboard-card's own proven card flags
# (scripts/build-rtgboard-card.sh), minus the P96 SDK include (this
# library needs only its own public headers and the toolchain's bundled
# NDK): -std=c99 -O2 -Wall -Wextra -mcpu=68020 -noixemul
# -fomit-frame-pointer.
CFLAGS=(-std=c99 -O2 -Wall -Wextra -mcpu=68020 -noixemul -fomit-frame-pointer \
        -I"$SRC_DIR/include" -I"$SRC_DIR")

# prometheus.library is an Exec AUTOINIT library, not a program: no C
# runtime (same reasoning as rtgboard-card's own LDFLAGS comment).
...
"$CC" "${CFLAGS[@]}" -c "$SRC_DIR/prometheus_library.c" -o "$BUILD/prometheus_library.o"

# debug.lib: KPrintF (clib/debug_protos.h), used for every LibInit/config-
# cycle/BAR-assignment narration marker (serial markers are treated as
# positive evidence in this project). This toolchain ships it under its
...
# also supplies the NextTagItem the library calls -- but note this
# toolchain's copy still references UtilityBase (confirmed by link failure
# without it), which is why prometheus_library.c opens utility.library in
# LibInit and keeps it open (see its UtilityBase comment).
"$CC" "${LDFLAGS[@]}" -o "$OUT" \
    "$BUILD/libstart.o" "$BUILD/prometheus_library.o" \
    -L"$NDK_LIBS_DIR" -l:debug.lib -lamiga

# Intermediate .o files are scratch, not part of the committed-binary policy
# above (only prometheus.library itself is checked in) -- clean them up so
...
NAME_COUNT=$(strings "$OUT" | grep -c "prometheus.library" || true)
if [ "$NAME_COUNT" -lt 1 ]; then
    echo "error: $OUT does not contain the string \"prometheus.library\"" >&2
    exit 1
fi
echo "verified: string \"prometheus.library\" present ($NAME_COUNT occurrence(s))"
